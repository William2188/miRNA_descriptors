...
for item in cancer_type:
    ...
    df = df_descriptor[1:]
    num_rows=df.shape[0]
    df.to_csv(item+'_descriptor_output.csv', index=False)
    ...
    if not (item == 'multi_disease'):
        ...
        df = df_descriptor.loc[range(num_rows)]
        df = df.loc[1:]
        df2 = df_id.loc[range(num_rows)]
        df2 = df2.loc[1:]
        #print(df)
        df.to_csv(item+'_descriptor_output.csv', mode ='a', index=False, header=False)
        df2.to_csv(item+'_non_associated_mirna_id.csv', index=False, header=True)
        print('output file: ' + item+'_descriptor_output.csv')
